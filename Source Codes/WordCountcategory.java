...
public class WordCountcategory {     
...
itr[0] = We  itr[1] = Orange  itr[2]=Mango
     ...
     // System.out.println(itr);
      while (itr.hasMoreTokens()) {
        ...
        int l = s.length();
        if(l==1) 
           context.write(new Text("tiny_category"),one);
        else if (l>=2 && l<=4)
            context.write(new Text("small_category"),one);
         else if (l>=5 && l<=9)
            context.write(new Text("medium_category"),one);
         else 
             context.write(new Text("big_category"),one);
  ...
  public static class IntSumReducer 
    ...
    /* In the following line, the first argument indicates a word in the input file. The second argument indicates a list of values. For example, consider "Apple" is a word in the input file public void map(Object key, Text value, Context context
    ) throws IOException, InterruptedException {and it occurs 5 times. Then, map function  gives this (Apple,[1,1,1,1]) as an output for the word "Apple". [1,1,1,1] is an iterable list. mango,[1,1] */
...
//otherArgs[0] = path of the input folder
//otherArgs[1] = path of the ouput folder
...
leOutputFormat
    ...
    job.setJarByClass(WordCountcategory.class);
